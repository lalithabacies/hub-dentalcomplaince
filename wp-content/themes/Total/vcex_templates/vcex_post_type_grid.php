							...
							elseif ( $atts['post_thumbnail'] ) :
								...
								$output .= '<div class="'. $media_classes .'">';
									...
									if ( $thumb_link == 'post' || $thumb_link == 'lightbox' ) :
										...
										$output .= '<a href="'. $entry_link .'" title="'. $atts['post_esc_title'] .'"'. $url_target .''. $entry_link_classes .'>';
											$output .= $entry_image;
											ob_start();
							...
							$output .= '<div class="'. $classes .'"'. $content_style .'>';
								...
								if ( 'true' == $title ) :
									...
									$output .= '<'. $title_tag .' class="vcex-post-type-entry-title entry-title" '. $heading_style .'>';
										$output .= '<a href="'. $atts['post_permalink'] .'" title="'. $atts['post_esc_title'] .'"'. $url_target .''. $heading_link_style .'>';
								...
								if ( 'true' == $date ) :
									...
									$output .= '<div class="vcex-post-type-entry-date"'. $date_style .'>';
										...
										if ( 'tribe_events' == $atts['post_type']
											&& function_exists( 'wpex_get_tribe_event_date' )
										) {
											$instance = $unique_id ? $unique_id : 'vcex_post_type_grid';
											$atts['post_date'] = wpex_get_tribe_event_date( $instance );

										// Get standard date
										} else {
											$atts['post_date'] = get_the_date();
										}

										// Output date
										$output .= $atts['post_date'];
								...
								if ( 'true' == $show_categories && taxonomy_exists( $categories_taxonomy ) ) :
									...
									$output .= '<div class="'. $categories_classes .'"'. $categories_style .'>';
										...
										if ( 'true' == $show_first_category_only ) {
											$output .= wpex_get_first_term_link( $atts['post_id'], $categories_taxonomy );
										} else {
