	...
	$output .= '<div class="'. $wrap_classes .'"'. vcex_get_unique_id( $unique_id ) .'>';
		...
		$output .= '<div class="'. $grid_classes .'"'. $grid_data .'>';
			...
			while ( $wpex_query->have_posts() ) :
				...
				if ( $filter_taxonomy ) {
					$post_terms = get_the_terms( $atts['post_id'], $filter_taxonomy );
					if ( $post_terms ) {
								...
								if ( 'true' == $title ) :
									...
									$output .= '<'. esc_attr( $title_tag ) .' class="staff-entry-title entry-title"'. $heading_style .'>';
										...
										if ( 'post' == $title_link ) :
											...
											$output .= '<a href="'. $atts['post_permalink'] .'" title="'. $atts['post_esc_title'] .'"'. $heading_link_style .''. vcex_html( 'target_attr', $link_target ) .'>'. $atts['post_title'] .'</a>';
										...
										elseif ( 'lightbox' == $title_link ) :
											...
											$output .= '<a href="'. wpex_get_lightbox_image() .'" title="'. $atts['post_esc_title'] .'"'. $heading_link_style .' class="wpex-lightbox">'. $atts['post_title'] .'</a>';
