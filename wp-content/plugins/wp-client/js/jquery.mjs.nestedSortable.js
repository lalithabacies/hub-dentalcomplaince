...
(function($) {
	...
	$.widget("mjs.nestedSortable", $.extend({}, $.ui.sortable.prototype, {
		...
		_mouseDrag: function(event) {
			...
			for (var i = this.items.length - 1; i >= 0; i--) {
				...
				if(itemElement != this.currentItem[0] //cannot intersect with itself
                    ...
                    if (this.options.isTree && $(itemElement).hasClass(this.options.collapsedClass) && this.options.expandOnHover) {
                        if (!this.hovering) {
                            $(itemElement).addClass(this.options.hoveringClass);
                            var self = this;
                            this.hovering = window.setTimeout(function() {
                                $(itemElement).removeClass(self.options.collapsedClass).addClass(self.options.expandedClass);
                                self.refreshPositions();
                                self._trigger("expand", event, self._uiHash());
                            }, self.options.expandOnHover);
                        }
                    }
					...
					if (this.options.tolerance == "pointer" || this._intersectsWithSides(item)) {
                        ...
                        $(itemElement).removeClass(this.options.hoveringClass);
			...
			// If the item is below a sibling and is moved to the right, make it a child of that sibling.
                ...
                if (!previousItem.children(o.listType).length) {
                    ...
                    previousItem.children('span.disclose').show();
				}
        ...
        getCurrentItem: function( options ) {
            var o = $.extend({}, this.options, options),
                sDepth = o.startDepthCount || 0,
                ret = [],
                left = 2;
            ...
            o.currentID = (o.currentID).match(o.expression || (/(.+)[-=_](.+)/));
            o.currentID = o.currentID[2];
