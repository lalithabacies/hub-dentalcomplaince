...
jQuery(document).ready(function() {
    ...
    jQuery( '.wpc_client_files_list' ).on( 'click', '.wpc_file_details_link', function() {
        var current_details = jQuery( this ).parents( '.wpc_filedata_wrapper' ).find( '.wpc_file_details' );

        var change_value = jQuery( this ).parents( '.wpc_client_files_list' ).find( '.wpc_file_details.wpc_visible' ).not( current_details ).parents( '.wpc_filedata_wrapper' ).find( '.wpc_file_details_link' ).data('hide_text');
        jQuery( this ).parents( '.wpc_client_files_list' ).find( '.wpc_file_details.wpc_visible' ).not( current_details ).parents( '.wpc_filedata_wrapper' ).find( '.wpc_file_details_link' ).data( 'hide_text', jQuery( this ).parents( '.wpc_client_files_list' ).find( '.wpc_file_details.wpc_visible' ).not( current_details ).parents( '.wpc_filedata_wrapper' ).find( '.wpc_file_details_link' ).html() ).html( change_value );

        jQuery( this ).parents( '.wpc_client_files_list' ).find( '.wpc_file_details.wpc_visible' ).not( current_details ).removeClass( 'wpc_visible' );
        //jQuery( this ).parents( '.wpc_client_files_list' ).find( '.wpc_file_details' ).not( current_details ).removeClass( 'wpc_visible' );
        current_details.toggleClass( 'wpc_visible' );

        var change_value = jQuery(this).data('hide_text');
        jQuery(this).data( 'hide_text', jQuery(this).html() ).html( change_value );
    });


    /*filters js*/
    jQuery( '.wpc_client_files_list .wpc_show_filters' ).click( function(e) {
    ...
    jQuery( '.wpc_client_files_list .wpc_filters_contect' ).click( function(e) {
        e.stopPropagation();
    });

    jQuery( '.wpc_client_files_list .wpc_filter_by' ).change( function() {
        var obj = jQuery(this);

        if( obj.val() == 'none' ) {
            obj.parents( '.wpc_client_files_list' ).find( '.wpc_files_filter' ).hide();
            return false;
        }
        ...
        var files_form_id = obj.parents( '.wpc_client_files_list' ).data( 'form_id' );

        if( obj.parents( '.wpc_client_files_list' ).hasClass( 'wpc_fileslu_shortcode' ) ) {

            var shortcode_type = 'fileslu';
            var php_data = window['wpc_fileslu_pagination' + files_form_id];

        } else if( obj.parents( '.wpc_client_files_list' ).hasClass( 'wpc_filesla_shortcode' ) ) {
        ...
        var search = obj.parents( '.wpc_client_files_list' ).find( '.wpc_files_search.wpc_searched' ).val();
        ...
        jQuery.ajax({
            ...
            success: function( data ){
                if( !data.status ) {
                    ...
                    obj.parents( '.wpc_client_files_list' ).find( '.wpc_msg_filter_selectors' ).html( data.filter_html ).show();
    ...
    if( jQuery( '.wpc_client_files_list .wpc_filter_by' ).length > 0 ) {
        jQuery( '.wpc_client_files_list .wpc_filter_by' ).trigger( 'change' );
    }


    //change filtering reload filestable
    jQuery( '.wpc_client_files_list .wpc_filters_wrapper' ).change( function() {
        var obj = jQuery(this);
        ...
        obj.parents( '.wpc_client_files_list' ).find('.wpc_overlay').show();
        jQuery('body').css('cursor', 'wait');
        jQuery.ajax({
            type: 'POST',
            url: php_data.ajax_url,
            data: 'action=wpc_files_shortcode_list_pagination&shortcode_type=' + shortcode_type + '&shortcode_data=' + jQuery.base64Encode( JSON.stringify( php_data.data ) ).replace( /\+/g, "-" ) + '&current_page=' + page_number + '&last_category_id=' + last_category_id + '&exclude_author=' + exclude_author + '&sorting=' + sorting + '&filters=' + filters + '&search=' + search + '&client_id=' + client_id + '&security=' + _wpnonce,
            dataType: "json",
            success: function( data ){
                if( !data.status ) {
                    alert( data.message );
                    ...
                    obj.parents( '.wpc_client_files_list' ).find( '.wpc_filelist' ).html( data.html );

                    if( data.filter_html != '' ) {
                        obj.parents( '.wpc_client_files_list' ).find( '.wpc_show_filters' ).prop( 'disabled', false );
                        obj.parents( '.wpc_client_files_list' ).find( '.wpc_filter_by' ).html( data.filter_html ).trigger( 'change' );
                    } else {
                        obj.parents( '.wpc_client_files_list' ).find('.wpc_show_filters').prop( 'disabled', true );
                    }


                    if( php_data.data.show_pagination ) {
                        obj.parents( '.wpc_client_files_list' ).find( '.files_pagination_block' ).data( 'page_number', 1 ).data( 'last_category_id', data.last_category_id );

                        if( !data.pagination ) {
                            obj.parents( '.wpc_client_files_list' ).find( '.files_pagination_block' ).hide();
                        } else {
                            obj.parents( '.wpc_client_files_list' ).find( '.files_pagination_block' ).show();
                        }
                    }
                ...
                obj.parents( '.wpc_client_files_list' ).find('.wpc_overlay').hide();
    ...
    //add filter
    jQuery( '.wpc_client_files_list .wpc_add_filter' ).click( function() {
        ...
        if( filter_by == 'creation_date' ) {
            ...
            jQuery.ajax({
                ...
                success: function( data ){
                    if( !data.status ) {
                        ...
                        obj.parents( '.wpc_client_files_list' ).find( '.wpc_filters_wrapper' ).append(
            ...
            add_filter( obj.parents( '.wpc_client_files_list' ), filter_by, filter_id );
        }
    ...
    /* Sorting */
    if( jQuery( '.wpc_client_files_list .wpc_sorting' ).length > 0 ) {
        jQuery( '.wpc_client_files_list .wpc_sorting' ).each( function() {
            sort[jQuery(this).parents( '.wpc_client_files_list' ).data( 'form_id' )] = jQuery(this).val();
        });
    }

    jQuery( '.wpc_client_files_list .wpc_add_sort' ).click( function(e) {
    ...
    jQuery( '.wpc_client_files_list .wpc_sort_contect' ).click( function(e) {
        e.stopPropagation();
    });


    jQuery( '.wpc_client_files_list .wpc_apply_sort' ).click( function() {
        var obj = jQuery(this);

        var sort_by = obj.parents( '.wpc_sort_contect' ).find( '.wpc_sorting' ).val();
        var form_id = obj.parents( '.wpc_client_files_list' ).data( 'form_id' );
        ...
        var page_number = obj.parents( '.wpc_client_files_list' ).find('.files_pagination_block').data( 'page_number' );
        var last_category_id = obj.parents( '.wpc_client_files_list' ).find('.files_pagination_block').data( 'last_category_id' );
        ...
        jQuery.ajax({
            ...
            success: function( data ){
                if( !data.status ) {
                    ...
                    obj.parents( '.wpc_client_files_list' ).find('.files_pagination_block').data( 'last_category_id', data.last_category_id );
                    init_tooltips();
                }
                obj.parents( '.wpc_client_files_list' ).find('.wpc_overlay').hide();
                jQuery('body').css( 'cursor', 'default' );
    ...
    jQuery( '.wpc_client_files_list .wpc_files_search_button' ).click( function() {
        var obj = jQuery(this);
        ...
        var search = jQuery.trim( obj.parents( '.wpc_client_files_list' ).find( '.wpc_files_search' ).val() );
        if( search == '' ) {
            return false;
        }
        ...
        jQuery.ajax({
            ...
            success: function( data ){
                if( !data.status ) {
                    ...
                    obj.parents( '.wpc_client_files_list' ).find( '.wpc_files_search' ).addClass( 'wpc_searched' );
                    obj.parents( '.wpc_client_files_list' ).find('.wpc_files_clear_search' ).show();
    ...
    jQuery( '.wpc_client_files_list .wpc_files_search' ).keyup( function(event) {
        if( event.keyCode == 13 && jQuery(this).val() != '' ) {
            jQuery(this).parents( '.wpc_client_files_list' ).find( '.wpc_files_search_button' ).trigger( 'click' );
            event.stopPropagation();
        }
    });

    jQuery( '.wpc_client_files_list .wpc_files_clear_search' ).click( function() {
        var obj = jQuery(this);
        ...
        jQuery.ajax({
            ...
            success: function( data ){
                if( !data.status ) {
                    ...
                    obj.parents( '.wpc_client_files_list' ).find( '.wpc_files_search' ).removeClass( 'wpc_searched' ).val('');
                    obj.parents( '.wpc_client_files_list' ).find( '.wpc_files_clear_search' ).hide();
    ...
    jQuery( '.wpc_client_files_list .files_pagination_block' ).click( function() {
        var obj = jQuery(this);
        var files_form_id = obj.parents( '.wpc_client_files_list' ).data( 'form_id' );
        ...
        jQuery.ajax({
            ...
            success: function( data ){
                if( !data.status ) {
                    ...
                    obj.parents( '.wpc_client_files_list' ).find( '.file_item:last' ).after( data.html );
    ...
    //remove filters
    jQuery( '.wpc_client_files_list' ).on( 'click', '.wpc_remove_filter', function() {
        var obj = jQuery(this);
    ...
    //add filter from tag
    jQuery( '.wpc_client_files_list' ).on( 'click', '.wpc_tag', function() {
        var obj = jQuery(this);

        if( !obj.parents( '.wpc_client_files_list' ).find( '.wpc_show_filters' ).prop('disabled') ) {
    ...
    jQuery( '.wpc_client_files_list' ).on( 'click', '.wpc_file_author_value', function() {
        var obj = jQuery(this);
