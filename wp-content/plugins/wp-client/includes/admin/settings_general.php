<?php
 if ( ! defined( 'ABSPATH' ) ) { exit; } if ( isset( $_POST['update_settings'] ) ) { if ( isset( $_POST['wpc_general'] ) ) { $settings = $_POST['wpc_general']; } else { $settings = array(); } do_action( 'wp_client_settings_update', $settings, 'general' ); do_action( 'wp_client_redirect', admin_url() . 'admin.php?page=wpclients_settings&msg=u' ); exit; } $wpc_general = $this->cc_get_settings( 'general' ); $wpc_currency = $this->cc_get_settings( 'currency' ); $wpc_custom_titles = $this->cc_get_settings( 'custom_titles' ); ?>
...
<form action="" method="post" name="wpc_settings" id="wpc_settings" >
	...
	<div class="postbox">
		<h3 class="hndle"><span><?php _e( 'Resource\'s Directory Settings', WPC_CLIENT_TEXT_DOMAIN ) ?></span></h3>
		<div class="inside">
			<table class="form-table">
				<tr valign="top">
					<th scope="row">
						<label for="resources_folder"><?php _e( 'Resource\'s Directory Path', WPC_CLIENT_TEXT_DOMAIN ) ?>:</label>
					</th>
					<td>
						<input type="text" name="wpc_general[resources_folder]" id="resources_folder" value="<?php echo ( isset( $wpc_general['resources_folder'] ) ) ?$wpc_general['resources_folder'] : '' ?>" /><br />
						<span class="description"><strong><?php _e( 'Important!!!', WPC_CLIENT_TEXT_DOMAIN ) ?> </strong><?php _e( 'Before change path please copy \'wpclient\' folder to new path', WPC_CLIENT_TEXT_DOMAIN ) ?>.</span><br />
						<span class="description"><?php _e( 'Current path', WPC_CLIENT_TEXT_DOMAIN ) ?>: <strong><?php echo $this->get_upload_dir() ?></strong>.</span><br />
						<span class="description"><?php _e( 'If field is empty - plugin uses default path', WPC_CLIENT_TEXT_DOMAIN ) ?>: <strong><?php $wp_uploads = wp_upload_dir(); echo $wp_uploads['basedir'] . DIRECTORY_SEPARATOR; ?></strong>.</span>
					</td>
				</tr>
			</table>
		</div>
	...
	<div class="postbox">
		<h3 class="hndle"><span><?php _e( 'Portal Settings', WPC_CLIENT_TEXT_DOMAIN ) ?></span></h3>
		<div class="inside">
			<table class="form-table">
                ...
                <tr valign="top">
					<th scope="row">
						<label for="exclude_pp_from_search"><?php printf( __( 'Exclude %s from global Wordpress Search', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['portal_page']['p'] ) ?>:</label>
					</th>
					<td>
						<select name="wpc_general[exclude_pp_from_search]" id="exclude_pp_from_search" style="width: 100px;">
							<option value="yes" <?php echo ( isset( $wpc_general['exclude_pp_from_search'] ) && $wpc_general['exclude_pp_from_search'] == 'yes' ) ? "selected" : '' ?> ><?php _e( 'Yes', WPC_CLIENT_TEXT_DOMAIN ) ?></option>
							<option value="no" <?php echo ( isset( $wpc_general['exclude_pp_from_search'] ) && $wpc_general['exclude_pp_from_search'] == 'no' ) ? "selected" : '' ?> ><?php _e( 'No', WPC_CLIENT_TEXT_DOMAIN ) ?></option>
						</select>
					</td>
				</tr>
				...
				<tr valign="top">
					<th scope="row">
						<label for="avatars_shapes"><?php _e( 'Avatar\'s shapes', WPC_CLIENT_TEXT_DOMAIN ) ?>:</label>
					</th>
					<td>
						<select name="wpc_general[avatars_shapes]" id="avatars_shapes" style="width: 100px;">
							<option value="square" <?php echo ( isset( $wpc_general['avatars_shapes'] ) && $wpc_general['avatars_shapes'] == 'square' ) ? "selected" : '' ?> ><?php _e( 'Square', WPC_CLIENT_TEXT_DOMAIN ) ?></option>
							<option value="circle" <?php echo ( isset( $wpc_general['avatars_shapes'] ) && $wpc_general['avatars_shapes'] == 'circle' ) ? "selected" : '' ?> ><?php _e( 'Circle', WPC_CLIENT_TEXT_DOMAIN ) ?></option>
						</select>
					</td>
				</tr>
	...
	<div class="postbox">
		...
		<div class="inside">
			<table class="form-table">
				...
				<?php $locations = get_registered_nav_menus(); if ( is_array( $locations ) && 0 < count( $locations ) ) { foreach( $locations as $key => $value ) { ?>
					    <tr valign="top">
						    <th scope="row">
							    <label for="custom_menu_logged_in"><?php echo $value ?> <span class="description"><?php _e( '(logged-in)', WPC_CLIENT_TEXT_DOMAIN ) ?></span>:</label>
						    </th>
						    <td>
							    <?php $nav_menus = wp_get_nav_menus( array( 'orderby' => 'name' ) ); $num_menus = count( array_keys( $nav_menus ) ); if( $num_menus > 0 ) { ?>
				...
				<tr valign="top">
					<th scope="row">
						<label for="hub_link_text"><?php _e( 'HUB page link in menu text', WPC_CLIENT_TEXT_DOMAIN ) ?>:</label>
					</th>
					<td>
