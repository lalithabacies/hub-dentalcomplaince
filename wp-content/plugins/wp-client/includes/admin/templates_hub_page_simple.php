<?php
 if ( ! defined( 'ABSPATH' ) ) { exit; } if (!$can_edit) do_action( 'wp_client_redirect', get_admin_url( 'index.php' ) ); $error = ""; if ( isset( $_REQUEST['update_hub_template'] ) ) { if ( !isset( $_REQUEST['hub_template']['name'] ) || empty( $_REQUEST['hub_template']['name'] ) ) { $error .= __('A Template Name is required.<br/>', WPC_CLIENT_TEXT_DOMAIN); } if ( empty( $error ) ) { $wpc_ez_hub_templates = $this->cc_get_settings( 'ez_hub_templates' ); if ( isset( $_REQUEST['id'] ) && '' != $_REQUEST['id'] ) { $tmp_id = $_REQUEST['id']; } else { $tmp_id = time(); } $wpc_ez_hub_templates[$tmp_id]['name'] = $_REQUEST['hub_template']['name']; $wpc_ez_hub_templates[$tmp_id]['general'] = $_REQUEST['hub_template']['general']; $wpc_ez_hub_templates[$tmp_id]['type'] = 'simple'; if( (int)$_REQUEST['hub_template']['priority'] >= 0 ) { $wpc_ez_hub_templates[$tmp_id]['priority'] = (int)$_REQUEST['hub_template']['priority']; } else { $wpc_ez_hub_templates[$tmp_id]['priority'] = ''; } $content = $_REQUEST['hub_template']['content'] ; $target_path = $this->get_upload_dir( 'wpclient/_hub_templates/' ); if ( is_dir( $target_path ) ) { $content_file = fopen( $target_path . $tmp_id . '_hub_content.txt', 'w+' ); fwrite( $content_file, $content ); fclose( $content_file ); $tabs_content_file = fopen( $target_path . $tmp_id . '_hub_tabs_content.txt', 'w+' ); fwrite( $tabs_content_file, $content ); fclose( $tabs_content_file ); } do_action( 'wp_client_settings_update', $wpc_ez_hub_templates, 'ez_hub_templates' ); do_action( 'wp_client_redirect', admin_url() . 'admin.php?page=wpclients_templates&tab=hubpage&action=edit_simple_template&id=' . $tmp_id . '&msg=u' ); exit; } } if ( 'add_simple_template' == $_GET['action'] ) $button_text = __( 'Add Simple HUB Template', WPC_CLIENT_TEXT_DOMAIN ); else $button_text = __( 'Update Simple HUB Template', WPC_CLIENT_TEXT_DOMAIN ); if ( isset( $_REQUEST['hub_template'] ) ) { $hub_template = $_REQUEST['hub_template']; } elseif ( isset( $_REQUEST['id'] ) && '' != $_REQUEST['id'] ) { $wpc_ez_hub_templates = $this->cc_get_settings( 'ez_hub_templates' ); $hub_template = isset( $wpc_ez_hub_templates[$_REQUEST['id']] ) ? $wpc_ez_hub_templates[$_REQUEST['id']]: array(); if( isset( $hub_template['type'] ) && 'ez' == $hub_template['type'] && 'edit_simple_template' == $_GET['action'] ) { do_action( 'wp_client_redirect', admin_url() . 'admin.php?page=wpclients_templates&tab=hubpage&action=edit_ez_template&id=' . $_REQUEST['id'] ); exit; } elseif( isset( $hub_template['type'] ) && 'advanced' == $hub_template['type'] && 'edit_simple_template' == $_GET['action'] ) { do_action( 'wp_client_redirect', admin_url() . 'admin.php?page=wpclients_templates&tab=hubpage&action=edit_advanced_template&id=' . $_REQUEST['id'] ); exit; } } ?>
