<?php
 if ( ! defined( 'ABSPATH' ) ) { exit; } global $wpdb; define( 'IFRAME_REQUEST', true ); wp_enqueue_script( 'jquery-ui-spinner' ); wp_register_style( 'wpc-jqueryui', $this->plugin_url . 'css/jqueryui/jquery-ui-1.10.3.css' ); wp_enqueue_style( 'wpc-jqueryui' ); wp_enqueue_script( 'accordion' ); wp_enqueue_style( 'customize-controls' ); wp_enqueue_script( 'wp-color-picker' ); wp_enqueue_style( 'wp-color-picker' ); include $this->plugin_dir . '/includes/class.customize.php'; $wpc_client_customize = new WPC_Client_Customize(); $for_client = ( isset( $_GET['for_client'] ) ) ? '&for_client=' . $_GET['for_client'] : ''; if ( isset( $_POST['wpc_scheme_settings'] ) && isset( $_POST['wpc_style_settings'] ) && is_array( $_POST['wpc_style_settings'] ) ) { $wpc_client_customize->save_style_settings( $_POST['wpc_scheme_settings'], $_POST['wpc_style_settings'] ); do_action( 'wp_client_redirect', admin_url() . 'admin.php?page=wpclients_customize&msg=u' . $for_client ); exit; } if ( isset( $_GET['reset'] ) && 'true' == $_GET['reset'] ) { $wpc_client_customize->reset_style_settings(); do_action( 'wp_client_redirect', admin_url() . 'admin.php?page=wpclients_customize&msg=r' . $for_client ); exit; } $args = array( 'role' => 'wpc_client', 'meta_key' => 'wpc_cl_hubpage_id', 'fields' => 'ID', ); $client = get_users( $args ); $excluded_clients = "'" . implode( "','", $this->cc_get_excluded_clients() ) . "'"; $all_clients = $wpdb->get_results( "SELECT u.ID as id, u.user_login as username
    FROM {$wpdb->users} u
    ...
    ", ARRAY_A ); if ( isset( $_GET['for_client'] ) && in_array( $_GET['for_client'], $client ) ) { $hub_id = get_user_meta( $_GET['for_client'], 'wpc_cl_hubpage_id', true ); if ( $this->permalinks ) { $iframe_url = $this->cc_get_slug( 'hub_page_id' ) . $hub_id; } else { $iframe_url = add_query_arg( array( 'wpc_page' => 'hub_preview', 'wpc_page_value' => $hub_id ), $this->cc_get_slug( 'hub_page_id', false ) ); } } elseif( isset( $client[0] ) ) { $hub_id = get_user_meta( $client[0], 'wpc_cl_hubpage_id', true ); if ( $this->permalinks ) { $iframe_url = $this->cc_get_slug( 'hub_page_id' ) . $hub_id; } else { $iframe_url = add_query_arg( array( 'wpc_page' => 'hub_preview', 'wpc_page_value' => $hub_id ), $this->cc_get_slug( 'hub_page_id', false ) ); } } else { $iframe_url = get_home_url(); } $style_schemes = $wpc_client_customize->get_style_schemes(); $style_schemes_keys = array_keys( $style_schemes ); ?>

<style type="text/css">

    .section_block li h3 span {
        font-size: 12px;
    }
...
<!--                <a class="back button" onclick='return confirm("<?php _e( 'Are you sure to reset Custom Styles?', WPC_CLIENT_TEXT_DOMAIN ) ?>");' href="<?php echo admin_url( 'admin.php?page=wpclients_customize&reset=true' ) ?>"><?php _e( 'Reset', WPC_CLIENT_TEXT_DOMAIN ) ?></a>-->
            ...
            <div tabindex="-1" class="wp-full-overlay-sidebar-content accordion-container">
                ...
                <div id="properties_editor_box" style="display: none;" >
                    <div id="properties_editor" >
                        <?php echo $wpc_client_customize->_editor_html() ?>
                    </div>
                </div>

                <div id="block_for_select_client">
                    <span><?php printf( __( 'For %s: ', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['s'] ) ?></span>
                    <select name="for_client" id="for_client">
                        <?php
 if ( isset( $_GET['for_client'] ) ) { $selected_client = $_GET['for_client'] ; } elseif( isset( $client[0] ) ) { $selected_client = $client[0]; } else { $selected_client = ''; } if ( is_array( $all_clients ) && 0 < count( $all_clients ) ) { foreach( $all_clients as $client ) { $selected = ( $client['id'] == $selected_client ) ? ' selected="selected"' : ''; echo '<option value="' . $client['id'] . '" ' . $selected . ' >' . $client['username'] . '</option>'; } } ?>
                    </select>
                </div>
                ...
                <div class="accordion-section" id="customize-info">
                    ...
                    <div class="accordion-section-content" style="display: none; padding: 0 0 15px 20px;">
                        <b><?php _e( 'Scheme', WPC_CLIENT_TEXT_DOMAIN ) ?>:</b>

                        <select id="select_scheme" name="wpc_scheme_settings[key]" >
                            <?php
 $sh_title = ''; foreach( $style_schemes as $key => $value ) { echo '<option value="' . $key . '">' . $value['title'] . '</option>'; if ( empty( $sh_title ) ) { $sh_title = $value['title']; } } ?>
                        </select>
                        <input type="hidden" name="wpc_scheme_settings[title]" id="scheme_title" value="<?php echo $sh_title ?>" >
                    ...
                    <div id="theme_header">
                        <?php
 $first_scheme_key = array_keys( $style_schemes ); if ( isset( $first_scheme_key[0] ) ) { echo $wpc_client_customize->get_sections_header( $first_scheme_key[0] ); } ?>
                    </div>

                </div>


                <div id="wpc-customize-theme-controls">
                    <?php
 echo $wpc_client_customize->get_sections( '_default_scheme' ); ?>
        ...
        <script type="text/javascript">
            ...
            var eID = '';
            var properties_editor = jQuery( '#properties_editor_box' ).html();

            jQuery( '#properties_editor_box' ).remove();
            ...
            jQuery( document ).ready( function() {
                ...
                */
                ...
                jQuery( '#select_sections' ).live( 'change', function() {
                    //hide editor block
                    jQuery( '.properties_editor_place' ).html( '' );
...
//                    jQuery( '.theme-name' ).html( jQuery( '#select_sections option:selected' ).text() );
                ...
                //select client
                jQuery( '#for_client' ).live( 'change', function() {
                    var client_id = jQuery( this ).val();
                    var req_uri = "<?php echo preg_replace( '/&for_client=[0-9]+|&msg=[^&]+/', '', $_SERVER['REQUEST_URI'] ); ?>&for_client=" + client_id;
                ...
                jQuery('iframe#my_preview').load(function(){
                    ...
                    jQuery( 'iframe#my_preview' ).contents().find( '#wpadminbar' ).css('display', 'none');
                    jQuery( 'iframe#my_preview' ).contents().find( '#page' ).css('margin-top', '0');
...
//                    jQuery( '#wpc-customize-theme-controls' ).html( '' );
                    jQuery( '#wpc-customize-theme-controls' ).hide();
...
//                            jQuery( ".accordion-section-content" ).hide();
                            ...
                            if ( data.status ) {
                                jQuery( '#wpc-customize-theme-controls' ).html( jQuery( this ).get_sections( data.content ) );

                                jQuery( '#theme_header' ).html( data.sections_header_content );
                                jQuery( '#wpc-customize-theme-controls' ).show();
                ...
                jQuery( '#wpc_section_save' ).live( 'click', function() {
                    ...
                    if ( '' != section_name && ' ' != section_name ) {
                        ...
                        if ( '' != section_id ) {
                            ...
                            jQuery( '#wpc-customize-theme-controls ul' ).hide();

                            jQuery( '#wpc-customize-theme-controls' ).append( html );
...
//                            jQuery( '#wpc-customize-theme-controls' ).append( html );
                ...
                jQuery( '#wpc_customize_add_element' ).live( 'click', function() {
                    ...
                    if ( '' != title && '' != css_id ) {
                        ...
                        jQuery( '#wpc-customize-theme-controls .section_block:visible' ).append( html );
                ...
                //delete element
                jQuery( '.wpc_element_button_delete' ).live( 'click', function(e) {
                ...
                //show editor
                jQuery( '.section_block .control-section h3' ).live( 'click', function() {
                    var old_eID = eID;
                    eID = jQuery( this ).parent().attr( 'data-eID' );

                    if ( jQuery( this ).parent().hasClass( 'open' ) ) {

                        //save if click on another section
                        if ( '' != old_eID && old_eID != eID ) {
                            jQuery( this ).editor_get_values( old_eID );
                        }

                        jQuery( '.properties_editor_place' ).html( '' );
                        jQuery( this ).parent().find( '.properties_editor_place' ).html( properties_editor );

                        jQuery( this ).parent().init_editor_elements();

                        jQuery( this ).editor_set_values( eID );
                    } else {

                        //save if click on the same section
...
<script type="text/javascript">
                    ...
                    jQuery( document ).ready( function() {
                        ...
                        var allProperties2 = new Array(
                         ...
                         var allProperties = {
                             <?php
 $i = 1; $count = count( $wpc_client_customize->all_properties ); foreach( $wpc_client_customize->all_properties as $key => $type ) { echo ( $i != $count ) ? "'{$key}' : '{$type}'," : "'{$key}' : '{$type}'"; $i++; } ?>
                             <?php ?>
                         };
                        ...
                        jQuery.fn.wpc_style_to_iframe = function() {
                            ...
                            jQuery( '#wpc-customize-theme-controls .control-section' ).each( function() {
                        ...
                        jQuery.fn.editor_get_values = function( element_key ) {
                            jQuery.each( allProperties, function( key, type  ) {
                                ...
                                if ( 'color' == type ) {
                                    ...
                                    if ( jQuery( '#editor_' + key ).attr( 'checked' ) ) {
                                        jQuery( '#' + element_key + '_' + key ).val( '1' );
                                    } else {
                                        jQuery( '#' + element_key + '_' + key ).val( '' );
                                    }
...
//
                                ...
                                if ( 'color' == type ) {
                                    ...
                                    if ( '1' == jQuery( '#' + element_key + '_' + key ).val() ) {
                                        jQuery( '#editor_' + key ).attr( 'checked', true )
                                    } else {
                                        jQuery( '#editor_' + key ).attr( 'checked', false )
                                    }
...
//
                        ...
                        jQuery( document ).on( 'change', '#editor_background-color-transparent', function() {
                            var css_id  = jQuery( '#' + eID + '_css_id' ).val();

                            if ( jQuery( this ).attr( 'checked' ) ) {
                                jQuery('iframe#my_preview').contents().find( css_id ).css( 'background-color', 'transparent' );
                            } else {
                                jQuery('iframe#my_preview').contents().find( css_id ).css( 'background-color', jQuery( '#editor_background-color' ).val() );
                            }

                        });


                        jQuery( document ).on( 'change', '.wpc_border_style', function() {
                            var css_id  = jQuery( '#' + eID + '_css_id' ).val();
                        ...
                        jQuery( document ).on( 'change', '.wpc_font_family', function() {
                            var css_id  = jQuery( '#' + eID + '_css_id' ).val();
                        ...
                        //display
                        jQuery( document ).on( 'change', '#editor_display', function() {
                           console.log( css_id );
                            var css_id  = jQuery( '#' + eID + '_css_id' ).val();
                        ...
                        //float
                        jQuery( document ).on( 'change', '#editor_float', function() {
                            var css_id  = jQuery( '#' + eID + '_css_id' ).val();
                        ...
                        //size units
                        jQuery( document ).on( 'change', '.wpc_size_unit', function() {
                            var css_id  = jQuery( '#' + eID + '_css_id' ).val();
                        ...
                        jQuery.fn.init_editor_elements = function() {
                            ...
                            jQuery('.wpc_colors').wpColorPicker({
                                change: function( event, ui ) {
                                    ...
                                    if ( 'background' == type ) {
                                        ...
                                        if ( 'checked' != jQuery( '#editor_background-color-transparent' ).attr( 'checked' ) ) {
                                ...
                                clear: function() {
                                    var type = jQuery( this ).prev( '.wpc_colors' ).attr( 'data-color-type' );
                                    var css_id = jQuery( '#' + eID + '_css_id' ).val();

                                    if ( 'background' == type ) {

                                        if ( 'checked' != jQuery( '#editor_background-color-transparent' ).attr( 'checked' ) ) {
                                            jQuery('iframe#my_preview').contents().find( css_id ).css( 'background-color', '' );
                                        }

                                    } else if ( 'border' == type ) {
                                        jQuery('iframe#my_preview').contents().find( css_id ).css( 'border-color', '' );

                                    } else if ( 'font' == type ) {
                                        jQuery('iframe#my_preview').contents().find( css_id ).css( 'color', '' );
                                    }
                                }
