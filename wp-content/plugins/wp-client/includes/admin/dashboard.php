...
<div class="wrap">
    ...
    <h2 class="nav-tab-wrapper wpc-nav-tab-wrapper">
        <?php
 $tabs = array( 'dashboard' => __( 'Dashboard', WPC_CLIENT_TEXT_DOMAIN ), ); if ( current_user_can( 'wpc_admin' ) || current_user_can( 'administrator' ) ) { if ( !$this->plugin['hide_system_status_tab'] ) { $tabs['system_status'] = __('System Status', WPC_CLIENT_TEXT_DOMAIN); } if ( !$this->plugin['hide_get_started_tab'] ) { $tabs['get_started'] = __( 'Get Started', WPC_CLIENT_TEXT_DOMAIN ); } } foreach ( $tabs as $name => $label ) { echo '<a href="' . admin_url( 'admin.php?page=wpclients&tab=' . $name ) . '" class="nav-tab '; if ( $current_tab == $name ) echo 'nav-tab-active'; echo '">' . $label . '</a>'; } ?>
    </h2>
