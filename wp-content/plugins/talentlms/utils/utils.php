...
function tl_sync_wp_tl_users($tl_users_to_wp, $wp_users_to_tl, $hard_sync, $all_users) {
	...
	foreach ($tl_users_to_wp as $user) {
		...
		$new_wp_user_id = wp_insert_user(array('user_login' => $user['login'], 'user_pass' => $user['login'], 'user_email' => $user['email'], 'first_name' => $user['first_name'], 'last_name' => $user['last_name']));
		if (is_array($custom_fields)) {
			foreach($custom_fields as $custom_field) {
...
function getUnitIconClass($unit_type)
{
	$iconClass = '';
	switch ($unit_type) {
		case 'Unit':
