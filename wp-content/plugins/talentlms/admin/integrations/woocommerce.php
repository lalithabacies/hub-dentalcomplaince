...
// Turn off all error reporting to suppress in_array() warning
error_reporting(0);

function integrateWooCommerce($courses_to_integrate) {
	...
	foreach ($courses as $course) {
		if(!$course['hide_from_catalog'] && $course['status'] == 'active' && in_array($course['id'], $_POST['tl-integrate-courses'])){
			if(!in_array($course['id'], $integrated_courses) || $_POST['tl-force-integrate']) {
				...
				update_post_meta($post_id, '_talentlms_course_id', $course['id']);
    ...
    foreach ($courses_to_integrate as $course) {
        if(!in_array($course, $integrated_courses)) {
            $integrated_courses[] = $course;
        }
    }
	tl_delete_cache_value('tl_woocom_integrated_courses');
	tl_add_cache_value(array('name'=>'tl_woocom_integrated_courses', 'value' => serialize($integrated_courses)));
...
integrateWooCommerce($courses_to_integrate);
