...
function tl_custom_registration_form() {
	...
	$output .= __('First name', 'talentlms');
	$output .= "<br />";
...
function tl_custom_registration_errors($errors, $sanitized_user_login, $user_email) {
	if (empty($_POST['password']) || !empty($_POST['password']) && trim($_POST['password']) == '') {
		$errors->add('password_error', __('<strong>ERROR</strong> Please enter a password', 'talentlms'));
	}
	if (empty($_POST['first_name']) || !empty($_POST['first_name']) && trim($_POST['first_name']) == '') {
		$errors->add('first_name_error', __('<strong>ERROR</strong> Please type a first name', 'talentlms'));
	}
	if (empty($_POST['last_name']) || !empty($_POST['last_name']) && trim($_POST['last_name']) == '') {
		$errors->add('last_name_error', __('<strong>ERROR</strong> Please type a last name', 'talentlms'));
	}
	$custom_fields = array();
	$custom_fields = tl_get_custom_fields();;
...
function tl_custom_registration_register($user_id) {
	...
	if(!empty($_POST['billing_first_name']) && empty($_POST['first_name'])) {
		update_user_meta( $user_id, 'first_name', trim($_POST['billing_first_name']));
	}
	if (!empty($_POST['last_name'])) {
		update_user_meta( $user_id, 'last_name', trim($_POST['last_name']));
	}
	if(!empty($_POST['billing_last_name']) && empty($_POST['last_name'])) {
		update_user_meta( $user_id, 'last_name', trim($_POST['billing_last_name']));
	}
	if (!empty($_POST['password'])) {
		wp_set_password($_POST['password'], $user_id);
	}
	if(empty($_POST['password']) && !empty($_POST['account_password'])) {
	...
	} else if ($_POST['account_password']) {
